...
2. **Core Specialization:** Verticals train independently. Exact class counts evolve per cohort — see the active cohort's CLAUDE.md and `curriculum/index.html`. **BA and QA share four consecutive blocks before splitting:** (1) a short *Combined Opener* (~5 classes covering role reality, spec-reading, FR/NFR, acceptance criteria, estimation); (2) a *Spec Stack* block (~6 classes — Anatomy, BRD, SRS, HLD/LLD, Writing Mechanics, Spec Teardown); (3) a *Diagrams* block (~7 classes — Use Case, BPMN, Flowchart, Swimlane, Activity, DFD, Sequence); (4) a *Quality & Compliance* block (~5 classes — Testing Fundamentals, Bug Reporting, Risk Management, Data Protection, Security/Observability/Telemetry). After all four shared blocks, BA and QA split into vertical-specific tracks (cohort-specific class counts; see active cohort's CLAUDE.md).
...
│   ├── ba-qa-diagrams/                   #   BA+QA shared block 3 — diagrams (Use Case/BPMN/Flowchart/Swimlane/Activity/DFD/Sequence)
│   ├── ba-qa-quality/                    #   BA+QA shared block 4 — quality &amp; compliance (Testing/Bugs/Risk/DataProtection/Security-Observability-Telemetry)
...
- **Lesson materials:** every class needs a `*-slides.html`. Only produce a `*-notes.html` if it adds substantial content beyond the slides (extended worked examples, exercise solutions, references, glossary). If notes would just restate the slides — skip them.
